...
public static class Chunk
{
    ...
    public static Vector3Int GetPosFromChunkCoordinates(ChunkData chunkData, int x, int y, int z)
    {
        if (InRange(chunkData, x) && InRangeHeight(chunkData, y) && InRange(chunkData, z))
        {
            return new Vector3Int(x, y, z);
        ...
        return chunkData.worldReference.GetPosFromChunkCoordinates(chunkData, chunkData.worldPosition.x + x, chunkData.worldPosition.y + y, chunkData.worldPosition.z + z);
    }
    ...
    public static MeshData GetChunkOverlayMeshData(ChunkData chunkData)
    {
        MeshData overlayMeshData = new MeshData(false);

        // Loop through each block in the chunk to determine if it should have an overlay
        LoopThroughTheBlocks(chunkData, (x, y, z) =>
        {
            BlockType blockType = chunkData.blocks[GetIndexFromPosition(chunkData, x, y, z)];
            
            // Check if the block is currently being broken
            if (IsBlockBeingBroken(chunkData, x, y, z))
            {
                // Add overlay mesh data for this block
                overlayMeshData = BlockHelper.GetOverlayMeshData(chunkData, x, y, z, overlayMeshData, blockType, chunkData.BlocksBeingBroken[new Vector3Int(x,y,z)].overlayProgress);
    ...
    // Helper function to determine if a block is being broken
    private static bool IsBlockBeingBroken(ChunkData chunkData, int x, int y, int z)
    {
        return chunkData.IsBlockBeingBroken(new Vector3Int(x, y, z));
    }
